...
public class p_reportMonthly {
    ...
    public String getData()
    {
        ...
        Query = "SELECT category, sum(opening) as opening, sum(closing) as closing, sum(incoming) as incoming, sum(sales) as sales, sum(price*sales) as total from p_monthly, p_stock  WHERE p_stock.Barcode = p_monthly.Barcode group by category";
        ...
        try
        {
            ...
            String Query7 = "update p_monthly set p_monthly.opening=p_monthly.closing, p_monthly.sales=0";
            PreparedStatement psmt_clo = con.prepareStatement(Query7);
            psmt_clo.executeUpdate();


           String Query_sal = "update p_monthly, p_stock set p_monthly.closing=p_stock.numberofbottles where p_stock.Barcode=p_monthly.barcode";
             PreparedStatement   psmt_sal = con.prepareStatement(Query_sal);
                psmt_sal.executeUpdate();

            String Query_sales="update p_monthly set p_monthly.sales=p_monthly.opening-p_monthly.closing+p_monthly.incoming ";
            ...
            A1.add(String.valueOf(final_total));
             ...
             String Query_set_incoming = "update p_monthly set incoming=0 ";
                psmt_sal = con.prepareStatement(Query_set_incoming);
                psmt_sal.executeUpdate();

            return w2E.writeExcel(A1, "C:\\Reports_p_monthly\\");
    ...
    {
        p_reportMonthly rM1 = new p_reportMonthly();
