...
public class p_reportDaily {
    ...
    public String getData()
    {
        ...
        Query = "SELECT category, sum(opening) as opening, sum(closing) as closing, sum(incoming) as incoming, sum(sales) as sales, sum(price*sales) as total from p_daily, p_stock  WHERE p_stock.Barcode = p_daily.Barcode group by category" ;
        ...
        try
        {
            ...
            Query7 = "update p_daily set p_daily.opening=p_daily.closing, p_daily.sales=0";
                psmt_clo = con.prepareStatement(Query7);

                psmt_clo.executeUpdate();


           Query_sal = "update p_daily, p_stock set p_daily.closing=p_stock.numberofbottles where p_stock.Barcode=p_daily.barcode";
                psmt_sal = con.prepareStatement(Query_sal);
                psmt_sal.executeUpdate();

            Query_sales="update p_daily set p_daily.sales=p_daily.opening-p_daily.closing+p_daily.incoming ";
            ...
            while(res.next())
            {
                //Barcode = res.getLong("p_stock.Barcode");
                Category = res.getString("Category");
                //Brand = res.getString("p_stock.Brand");
                //Product = res.getString("p_stock.ProductName");
                //Volume = res.getInt("p_stock.Litres");
                Opening = res.getInt("opening");
                Closing = res.getInt("closing");
                Incoming = res.getInt("Incoming");
                //Price = res.getDouble("p_stock.Price");
            ...
            String Query_dis = "SELECT sum(discount) FROM `billingtable` WHERE is_parallel=1 and billid like '"+d1+"'";
            ...
            System.out.println(A1);
           String Query_set_incoming = "update p_daily set incoming=0 ";
                psmt_sal = con.prepareStatement(Query_set_incoming);
                psmt_sal.executeUpdate();

            return w2E.writeExcel(A1, "C:\\Reports_p_daily\\");
    ...
    {
        p_reportDaily rD1 = new p_reportDaily();
