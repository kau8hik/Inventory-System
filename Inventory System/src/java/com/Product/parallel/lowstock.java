...
public class lowstock {
    ...
    public ArrayList lowstock1(String Brand, String Category, String no){
       ...
       if((Category.equals("All")) && (Brand.equals("All")))
        {
            Query = "SELECT * FROM p_stock where numberofbottles<"+no;

        }
        else if(Category.equals("All") && (!Brand.equals("All")))
        {
            Query = "SELECT * FROM p_stock WHERE Brand = '"+Brand+"' and numberofbottles<"+no;
        }
        else if(Brand.equals("All") && (!Category.equals("All")))
        {
            Query = "SELECT * FROM p_stock WHERE Category = '"+Category+"' and numberofbottles<"+no;
        }
        else
        {
            Query = "SELECT * FROM p_stock WHERE Category = '"+Category+"' AND Brand = '"+Brand+"' And numberofbottles<"+no;
        }
